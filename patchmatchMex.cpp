...
void help() {
   mexErrMsgTxt("patchmatch Matlab wrapper\n\n"
         "[nnf,cost] = patchmatch(u1, u2, [w(5)], [minoff(0)], maxoff(imsize)], [iterations(5)], [randomtrials(5)], [method('SSD')], [nnfin])\n"
         "   \n"
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    if (nrhs>=i) method = mxArrayToString(prhs[i-1]);
    }
    ...
    void patchmatch(float *u1_, int nc  , int nr  , int nch, 
                ...
                int iterations, int randomtrials, bool planes);
